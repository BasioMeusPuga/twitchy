...
video_player=mpv
mpv_hardware_acceleration=no 		#Uses vaapi if enabled
quality=high 		#Valid options are: low / medium / high / source
...
if [[ "$day" = 0 ]]; then
	time_watched_hms="$(printf "%02d" $hour)h $(printf "%02d" $min)m $(printf "%02d" $sec)s"
else
	time_watched_hms="$(printf "%02d" $day)d $(printf "%02d" $hour)h $(printf "%02d" $min)m $(printf "%02d" $sec)s"
fi
}
...
total_followed=$(sed 's/,/\n/g' /tmp/twitchy | grep "_total" | cut -d ":" -f2)
curl -s "https://api.twitch.tv/kraken/users/$user_id/follows/channels?limit=$total_followed" > /tmp/twitchy

sed 's/,/\n/g' /tmp/twitchy | grep \"name\" | grep -v \"status\" | cut -d ":" -f2- | tr -d "\"" | sort > /tmp/twitchynew
...
comm -1 -3 /tmp/twitchyalreadyfollowed /tmp/twitchynew > /tmp/twitchyadd
	new_additions=$(wc -l < /tmp/twitchyadd)
...
if [[ $1 = "-n" ]]; then
	...
	while [[ $(wc -l < /tmp/twitchyfinal) != $totalstreams ]]
	do
		sleep .1
	done 2> /dev/null
...
while read line
	...
	now_online=$(sed ':a;N;$!ba;s/\n/ | /g' /tmp/twitchynowonline)
	if [[ $now_online != "" ]]; then
...
do
	...
	if [[ $game_name != "offline" ]] && [[ $(echo $game_name | cut -d "^" -f1) != "offline" ]] && [[ $game_name != "" ]]; then
		...
		if [[ $fav_mode != 1 ]]; then
			...
			game_name='\E[96m'$game_name'\E[0m'" - "'\E[92m'$stream_status'\E[0m'
			echo -ne " "'\E[93m'$a_var'\E[0m'
...
do
	...
	*)
	...
	if [[ $custom_quality = 1 ]]; then
		now_watching=$now_watching" "${final_selection[$check_channels]}" - "${final_quality[$check_channels]}" |"
	else
		now_watching=$now_watching" "${final_selection[$check_channels]}" |"
	fi
done
